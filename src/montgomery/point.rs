...
impl Point {
    ...
    fn core_add(&self, p: &Point) -> Point {
        ...
        let t9 = t7.add_nomod(&t8);
        let ta = t9.add_nomod(&(&t0 * a_ec));
        let rr = t5.add_nomod(&t6);
        let tt = ta.sub_nomod(&t1);
        let vv = (t9 * a_ec).add_nomod(&t0).add_nomod(&t0).add_nomod(&t0).add_nomod(&t2);
        let ss = (t3.sub_nomod(&t4) * s_ec).add_nomod(&t0).sub_nomod(&t2);
        let uu = (t7.sub_nomod(&t8) * s_ec).sub_nomod(&t3).sub_nomod(&t4);
        let ww = (t5.sub_nomod(&t6) * s_ec).add_nomod(&ta).add_nomod(&t1);
        let x3 = &rr * &ss - &tt * &uu;
        let y3 = tt * &ww - &vv * &ss;
        let z3 = vv * uu - rr * ww;
        self.e.new_proy_point(ProyCoordinates {
